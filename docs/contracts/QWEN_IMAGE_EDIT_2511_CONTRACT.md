...
- [ ] **Q1.9 Q4-RESIDENT strategy (SUPERSEDES PRELOAD-DEQUANT)** — amended 2026-04-22 per Q2 Phase 2 smoke RED verdict (`docs/qie_q2_p2_smoke.md`, commit `e0af05a7`). **Contract HBM budget was catastrophically wrong**: F16-dequant upload needs 40.86 GiB (1933 tensors), but 910B4 HBM is 32 GiB — **OOM at 26.3 GiB in smoke**. Missing from original estimate: per-block `img_mod` + `txt_mod` (13.6 GB) + `img_mlp` + `txt_mlp` (18 GB). **New strategy: keep Q4_0 weights resident on device (~5 GiB); dispatch matmul via `aclnnWeightQuantBatchMatmulV3` with `antiquantGroupSize=32` matching Q4_0 block layout.** Preload-dequant code in commit `0f860c5b` to be replaced, not added onto. **Gate 0 (probe)**: 2-4 hr WQBMMv3 W4 capability probe on ac03 — confirm op accepts `[K/32]` per-group F16 scale tensor at image-workload shapes. If GREEN, re-scope Phase 2 to Q4-resident upload + Phase 3 forward dispatches WQBMMv3 at each matmul site. If RED, re-open 4-option decision surface (A16W8 shrink or vendor ask). **Re-introduces per-step-D2H/H2D problem**: mitigated by Q2 aclGraph capture (step-keyed graph reuses the same Q4 weight pointer; no recopy).

**Gate**: end-to-end native-engine generates valid output image on canonical task. steps/s within ~20% of ggml-cann baseline (not a lift yet — just parity proven on native path). RoPE pre-compute verified at session init (not re-computed per step). **Q4-resident strategy verified to fit HBM with margin for activations (target peak ≤ 18 GiB incl. scratch).**
