...
- **CannFusion as the Ascend CUTLASS-analogue** (community, in-progress): on NVIDIA, CUTLASS is an open-source C++ template library of GEMM primitives + epilogues + autotuning that has become the de-facto "assembly layer" for custom CUDA kernels — agents read CUTLASS source directly, compose fused kernels without authoring raw PTX, and pattern-match examples in HuggingFace/DeepSpeed/xFormers derivatives. That agent-friendliness is a significant slice of why NVIDIA kernel authoring is tractable by agents today. **CannFusion** (Apache-2.0, `gitcode.com/Rust4CANN/CannFusion`, v0.2.5, ~147 stars) aspires to play the same role for Ascend: Rust-hosted codegen that emits AscendC operator directories (`kernel.h`, `kernel_entry.cpp`, `tiling.cpp`, `binary.json`, ACLNN two-phase host API) for GEMM + fused epilogues. We probed it extensively this project — F0 toolchain compat on CANN 8.3.RC1 / 910B4 proven GREEN, but F1 blocked on a hardcoded dtype whitelist at `src/validate.rs:141-163` that rejects A16W8 (our production dtype) with an explicit negative unit test. Upstream GitCode issue #26 filed; maintainer triaged and decomposed into four sub-issues (#27 weight-scale tensor model, #28 single-expert codegen, #29 mixed-dtype smoke, #30 Huawei-ref parity). Timeline: 2-3 weeks to months until upstream ships A16W8 single-expert. When it does, we unlock **~+0.6 fps dispatch-saving standalone plus possibly more from HBM round-trip elimination** on the CP FFN chain, plus a reusable authoring primitive for every future Ascend workload. **Honest caveat**: v0.2.5 pre-release, small maintainer team; adoption risk is real and mitigation is Apache-2.0 vendor-ability if upstream stalls. The CUTLASS analogy is aspirational, not current-state — but the fact that we're filing upstream issues to shape it means we're not just observing the DX gap, we're helping close it.
...
1. **Probe-first discovery** — 30-60 min cheap probes before funding tracks. Receipts: F0 CannFusion toolchain (1 hr), G0 aclGraph feasibility (1 hr), FO-audit of 717 headers (30 min), GD-audit RVQ confirmation (1 hr), A16W8-supplement audit (30 min).
