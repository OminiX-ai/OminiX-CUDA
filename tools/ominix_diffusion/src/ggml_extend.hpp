...
#if defined(SD_USE_CUDA) || defined(SD_USE_SYCL) || defined(SD_USE_CANN)
...
protected:
    ...
    void copy_data_to_backend_tensor() {
        for (auto& kv : backend_tensor_data_map) {
            ...
            // NaN diagnostic: check CPU source data before copy
            if (getenv("SD_NAN_CHECK") && tensor->type == GGML_TYPE_F32) {
                int64_t n = ggml_nelements(tensor);
                const float* fdata = (const float*)data;
                int nans = 0;
                float mn = fdata[0], mx = fdata[0];
                for (int64_t j = 0; j < n; j++) {
                    if (std::isnan(fdata[j]) || std::isinf(fdata[j])) { nans++; continue; }
                    if (fdata[j] < mn) mn = fdata[j];
                    if (fdata[j] > mx) mx = fdata[j];
                }
                LOG_INFO("[COPY CHECK] %s: CPU src name='%s' shape=[%ld,%ld,%ld,%ld] min=%.4f max=%.4f nans=%d/%ld",
                         get_desc().c_str(), ggml_get_name(tensor),
            ...
            // NaN diagnostic: verify NPU data after copy
            if (getenv("SD_NAN_CHECK") && tensor->type == GGML_TYPE_F32) {
                int64_t n = ggml_nelements(tensor);
                std::vector<float> readback(n);
                ggml_backend_tensor_get(tensor, readback.data(), 0, n * sizeof(float));
                int nans = 0;
                float mn = readback[0], mx = readback[0];
                for (int64_t j = 0; j < n; j++) {
                    if (std::isnan(readback[j]) || std::isinf(readback[j])) { nans++; continue; }
                    if (readback[j] < mn) mn = readback[j];
                    if (readback[j] > mx) mx = readback[j];
                }
                if (nans > 0) {
                    LOG_INFO("[COPY CHECK] %s: NPU readback MISMATCH name='%s' nans=%d/%ld min=%.4f max=%.4f",
                             get_desc().c_str(), ggml_get_name(tensor), nans, (long)n, mn, mx);
                }
            }
        }
...
public:
    ...
    bool compute(get_graph_cb_t get_graph,
        ...
        // NaN diagnostic: check leaf tensors BEFORE compute
        if (getenv("SD_NAN_CHECK") && !ggml_backend_is_cpu(runtime_backend)) {
            ggml_backend_synchronize(runtime_backend);
            int n_nodes = ggml_graph_n_nodes(gf);
            // Check the first RESHAPE node's source (leaf) to see if data is intact pre-compute
            for (int i = 0; i < std::min(n_nodes, 3); i++) {
                ...
                for (int64_t j = 0; j < n; j++) {
                    if (std::isnan(buf[j]) || std::isinf(buf[j])) { nans++; continue; }
                    if (buf[j] < mn) mn = buf[j];
                    if (buf[j] > mx) mx = buf[j];
                }
                LOG_INFO("[PRE-COMPUTE] %s: node %d op=%s name='%s' shape=[%ld,%ld,%ld,%ld] min=%.4f max=%.4f nans=%d/%ld",
                         get_desc().c_str(), i, ggml_op_name(node->op), ggml_get_name(node),
        ...
        ggml_graph_print(gf);
#endif
        // NaN detection: binary search for the first op that PRODUCES NaN
        // Since graph allocator reuses buffers, we use a two-pass approach:
        // 1. Coarse scan: check every 100th node to find approximate range
        // 2. Fine scan: check every node in the narrowed range
        if (getenv("SD_NAN_CHECK") && !ggml_backend_is_cpu(runtime_backend)) {
            int n_nodes = ggml_graph_n_nodes(gf);
            std::vector<float> buf;

            // Helper lambda to check a node for NaN
            auto check_node = [&](int i) -> int {
                struct ggml_tensor* node = ggml_graph_node(gf, i);
                if (node->type != GGML_TYPE_F32) return -1;  // skip
                int64_t n = ggml_nelements(node);
                buf.resize(n);
                ggml_backend_tensor_get(node, buf.data(), 0, n * sizeof(float));
                int nans = 0;
                for (int64_t j = 0; j < n; j++) {
                    if (std::isnan(buf[j]) || std::isinf(buf[j])) nans++;
                }
                return nans;
            };

            // Coarse scan: every 100 nodes
            int coarse_start = -1;
            for (int i = 0; i < n_nodes; i += 100) {
                int nans = check_node(i);
                if (nans > 0) {
                    coarse_start = (i >= 100) ? i - 100 : 0;
                    LOG_INFO("[NaN SCAN] %s: coarse hit at node %d/%d, scanning [%d..%d]",
                             get_desc().c_str(), i, n_nodes, coarse_start, i);
                    break;
                }
            }

            // Fine scan in the narrowed range
            if (coarse_start >= 0) {
                int fine_end = std::min(coarse_start + 200, n_nodes);
                for (int i = coarse_start; i < fine_end; i++) {
                    struct ggml_tensor* node = ggml_graph_node(gf, i);
                    if (node->type != GGML_TYPE_F32) continue;
                    int64_t n = ggml_nelements(node);
                    buf.resize(n);
                    ...
                    LOG_INFO("[NaN FINE] %s: node %d/%d op=%s name='%s' shape=[%ld,%ld,%ld,%ld] "
                             "nans=%d/%ld min=%.4f max=%.4f",
                    ...
                    if (nans > 0) {
                        for (int s = 0; s < GGML_MAX_SRC; s++) {
                            if (node->src[s]) {
                                LOG_INFO("[NaN FINE]   src[%d]: op=%s name='%s' shape=[%ld,%ld,%ld,%ld] type=%s",
                                         s, ggml_op_name(node->src[s]->op), ggml_get_name(node->src[s]),
                                         (long)node->src[s]->ne[0], (long)node->src[s]->ne[1],
                                         (long)node->src[s]->ne[2], (long)node->src[s]->ne[3],
                                         ggml_type_name(node->src[s]->type));
                            }
                        }
                    }
                }
