...
public:
    ...
    ggml_backend_t diffusion_backend   = nullptr;  // [Phase2] for per-component NPU isolation
    ...
    ~StableDiffusionGGML() {
        ...
        if (diffusion_backend != nullptr && diffusion_backend != backend) {
    ...
    void init_backend() {
        ...
        if (!backend) {
            LOG_WARN("Failed to initialize CANN backend");
        }
#endif
    ...
    bool init(const sd_ctx_params_t* sd_ctx_params) {
...

        {
            ...
            // [Phase2] Per-component NPU isolation via SD_NPU_COMPONENT env var
            // Values: "encoder" | "diffusion" | "vae" | "all" (default) | "none"
            diffusion_backend = backend;  // default: same as main backend
            const char* npu_comp_env = getenv("SD_NPU_COMPONENT");
            if (npu_comp_env && !ggml_backend_is_cpu(backend)) {
                std::string npc(npu_comp_env);
                LOG_INFO("[Phase2] SD_NPU_COMPONENT=%s — selective NPU backend mode", npu_comp_env);
                if (npc == "encoder") {
                    if (clip_backend != backend) { ggml_backend_free(clip_backend); }
                    clip_backend = backend;  // encoder on NPU
                    diffusion_backend = ggml_backend_cpu_init();
                } else if (npc == "diffusion") {
                    if (!ggml_backend_is_cpu(clip_backend)) {
                        clip_backend = ggml_backend_cpu_init();
                    }
                    diffusion_backend = backend;  // diffusion on NPU
                } else if (npc == "vae") {
                    if (!ggml_backend_is_cpu(clip_backend)) {
                        clip_backend = ggml_backend_cpu_init();
                    }
                    diffusion_backend = ggml_backend_cpu_init();
                    // vae will be forced to NPU below
                } else if (npc == "none") {
                    if (!ggml_backend_is_cpu(clip_backend)) {
                        clip_backend = ggml_backend_cpu_init();
                    }
                ...
                // "all" or unrecognized = all on NPU (default)
            }
            ...
            if (sd_version_is_sd3(version)) {
                ...
                if (sd_ctx_params->diffusion_conv_direct) {
                    LOG_INFO("Using Conv3d direct in the Wan diffusion model");
                    std::dynamic_pointer_cast<WanModel>(diffusion_model)->set_conv3d_direct_enabled(true);
                    if (high_noise_diffusion_model) {
                        std::dynamic_pointer_cast<WanModel>(high_noise_diffusion_model)->set_conv3d_direct_enabled(true);
                    }
                }
            ...
            // [Phase2] Override VAE backend based on SD_NPU_COMPONENT
            {
                const char* npu_comp_vae = getenv("SD_NPU_COMPONENT");
                if (npu_comp_vae && !ggml_backend_is_cpu(backend)) {
                    std::string npc(npu_comp_vae);
                    if (npc == "vae") {
                        // Force VAE on NPU (override --vae-on-cpu)
                        if (vae_backend != backend) {
                            ggml_backend_free(vae_backend);
                        }
                        vae_backend = backend;
                        LOG_INFO("[Phase2] VAE forced to NPU");
                    } else if (npc == "encoder" || npc == "diffusion" || npc == "none") {
                        // Force VAE on CPU
                        if (!ggml_backend_is_cpu(vae_backend)) {
                            vae_backend = ggml_backend_cpu_init();
                        }
                        LOG_INFO("[Phase2] VAE forced to CPU");
                    }
                }
            }
            ...
            if (!(use_tiny_autoencoder || version == VERSION_SDXS) || tae_preview_only) {
                if (sd_version_is_wan(version) || sd_version_is_qwen_image(version) || sd_version_is_anima(version)) {
                    ...
                    if (sd_ctx_params->vae_conv_direct) {
                        ...
                        first_stage_model->set_conv3d_direct_enabled(true);
                    }
...
// [Phase2] NaN/Inf check helper for debugging CANN backend
static bool check_tensor_nan(struct ggml_tensor* t, const char* label) {
    if (t == nullptr) return false;
    int64_t n = ggml_nelements(t);
    if (t->type != GGML_TYPE_F32) {
        LOG_WARN("[NaN CHECK] %s: skipping non-f32 tensor (type=%d, n=%" PRId64 ")", label, t->type, n);
        return false;
    }
    ...
    for (int64_t i = 0; i < n; i++) {
        ...
        else { min_val = std::min(min_val, data[i]); max_val = std::max(max_val, data[i]); }
    }
    if (nan_count > 0 || inf_count > 0) {
        LOG_ERROR("[NaN CHECK] %s: %" PRId64 " elements — %d NaN, %d Inf, range=[%f, %f]",
                  label, n, nan_count, inf_count, min_val, max_val);
        return true;
    }
    LOG_INFO("[NaN CHECK] %s: OK (%" PRId64 " elements, range=[%.6f, %.6f])", label, n, min_val, max_val);
    return false;
}
...
sd_image_t* generate_image_internal(sd_ctx_t* sd_ctx,
    ...
    // [Phase2] NaN check after text encoding
    ...
    for (size_t i = 0; i < final_latents.size(); i++) {
        ...
        if (img != nullptr) {
            // [Phase2] NaN check after VAE decode
            check_tensor_nan(img, "vae/decoded_image");
...
SD_API sd_image_t* generate_video(sd_ctx_t* sd_ctx, const sd_vid_gen_params_t* sd_vid_gen_params, int* num_frames_out) {
    ...
    // Sample
    {
        ...
        int64_t sampling_end = ggml_time_ms();
        LOG_INFO("sampling completed, taking %.2fs", (sampling_end - sampling_start) * 1.0f / 1000);
        check_tensor_nan(final_latent, "video/diffusion_output");
        if (sd_ctx->sd->free_params_immediately) {
            sd_ctx->sd->diffusion_model->free_params_buffer();
        }
    }
    ...
    check_tensor_nan(vid, "video/decoded_output");
    if (sd_ctx->sd->free_params_immediately && !sd_ctx->sd->use_tiny_autoencoder) {
        sd_ctx->sd->first_stage_model->free_params_buffer();
    }
