...
struct SDCliParams {
    ...
    ArgOptions get_options() {
        ...
        options.int_options = {
            ...
            {"",
             "--benchmark",
             "run N rounds of generation for benchmarking (excludes 1 warmup round), 0 to disable (default: 0)",
             &benchmark_rounds},
        };
...
static void print_benchmark_stats(const std::vector<double>& times_ms) {
    int n = (int)times_ms.size();
    if (n == 0)
        return;

    double sum = 0, min_t = times_ms[0], max_t = times_ms[0];
    ...
    LOG_INFO("rounds:     %d", n);
    LOG_INFO("min:        %.2fs", min_t / 1000.0);
    LOG_INFO("max:        %.2fs", max_t / 1000.0);
    LOG_INFO("avg:        %.2fs", avg / 1000.0);
    LOG_INFO("stddev:     %.2fs", stddev / 1000.0);
    LOG_INFO("throughput: %.3f images/sec", 1000.0 / avg);
    LOG_INFO("=======================================");
}
...
int main(int argc, const char* argv[]) {
    ...
    bool free_params = cli_params.benchmark_rounds <= 0;
    sd_ctx_params_t sd_ctx_params = ctx_params.to_sd_ctx_params_t(vae_decode_only, free_params, cli_params.taesd_preview);
    ...
    if (cli_params.mode == UPSCALE) {
        ...
        if (cli_params.mode == IMG_GEN) {
            ...
            if (cli_params.benchmark_rounds > 0) {
                // Warmup run
                LOG_INFO("benchmark warmup run...");
                sd_image_t* warmup_results = generate_image(sd_ctx, &img_gen_params);
                if (warmup_results != nullptr) {
                    for (int i = 0; i < gen_params.batch_count; i++) {
                        free(warmup_results[i].data);
                    }
                    free(warmup_results);
                }
                LOG_INFO("benchmark warmup completed");

                // Benchmark rounds
                std::vector<double> times_ms;
                for (int r = 0; r < cli_params.benchmark_rounds; r++) {
                    int64_t t_start = ggml_time_ms();
                    sd_image_t* round_results = generate_image(sd_ctx, &img_gen_params);
                    int64_t t_end = ggml_time_ms();
                    times_ms.push_back((double)(t_end - t_start));
                    LOG_INFO("benchmark round %d/%d: %.2fs", r + 1, cli_params.benchmark_rounds, times_ms.back() / 1000.0);

                    if (r == cli_params.benchmark_rounds - 1) {
                        // Keep last round results for saving
                        results = round_results;
                    } else if (round_results != nullptr) {
                        for (int i = 0; i < gen_params.batch_count; i++) {
                            free(round_results[i].data);
                        }
                        free(round_results);
                    }
                }
                print_benchmark_stats(times_ms);
            } else {
            ...
            if (cli_params.benchmark_rounds > 0) {
                ...
                LOG_INFO("benchmark warmup run...");
                int warmup_num = 0;
                sd_image_t* warmup_results = generate_video(sd_ctx, &vid_gen_params, &warmup_num);
                if (warmup_results != nullptr) {
                ...
                for (int r = 0; r < cli_params.benchmark_rounds; r++) {
                    int64_t t_start = ggml_time_ms();
                    int round_num = 0;
                    sd_image_t* round_results = generate_video(sd_ctx, &vid_gen_params, &round_num);
                    int64_t t_end = ggml_time_ms();
                    times_ms.push_back((double)(t_end - t_start));
                    LOG_INFO("benchmark round %d/%d: %.2fs", r + 1, cli_params.benchmark_rounds, times_ms.back() / 1000.0);
